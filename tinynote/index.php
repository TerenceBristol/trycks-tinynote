...
<head>
  ...
  <link rel="apple-touch-startup-image" href="assets/images/bg.png" media="(device-width: 320px) and (device-height: 568px)">
...
<body class="notes-app">
  ...
  <div class="editor-wrap">
    <!-- Removed data-placeholder="Note" -->
    <div id="noteBody" class="note-body" contenteditable="true"></div>
  </div>
  ...
  <script>
    ...
    // Place caret at end & focus (helps ensure the on-screen keyboard opens on mobile)
    function focusNote() {
      if (!noteDiv || !noteDiv.isContentEditable) return;
      // Focus first; some mobile browsers require this before selection changes
      noteDiv.focus({ preventScroll: true });
    ...
    // One-time global tap/click to focus editor and open keyboard
    function armGlobalFirstTapFocus() {
      const once = () => {
        // If user actually tapped inside note already, leave caret mgmt to browser
        if (document.activeElement !== noteDiv) focusNote();
        document.removeEventListener('pointerdown', once, { capture: true });
        document.removeEventListener('touchstart', once, { capture: true });
        document.removeEventListener('click', once, { capture: true });
      };
      // Use capture so we run before other handlers; passive to stay smooth
    ...
    // On desktop, focus immediately; on iOS/Android the keyboard won’t open
    // until a user gesture, which the one-time listeners above handle.
    window.addEventListener('DOMContentLoaded', () => {
      if (!/Mobi|Android|iPhone|iPad/i.test(navigator.userAgent)) {
        focusNote();
      }
      armGlobalFirstTapFocus();
    });

    // Save with offline queue + (3) skip if unchanged
